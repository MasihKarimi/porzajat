...
<div class="layout-header">
    <div class="navbar navbar-default">
        <div class="navbar-header">
            ...
            <a class="navbar-brand" style="color:#ffffff;background-color:#232b2b; font-size: medium;align-content: center">
                &blacktriangleleft;
                <span style="font-size: 25pt;color: #ffffff; " class="sidenav-icon icon icon-gear"></span>
                Hamid Lemar Ltd &blacktriangleright;
            </a>
            ...
            <nav id="navbar" class="navbar-collapse collapse">
                ...
                <ul class="nav navbar-nav navbar-right">
                    <li class="visible-xs-block">
                        <h4 class="navbar-text text-center">Powered By : <a href="https://zeersign.com"> ZeerSign Software Solutions </a></h4>
                    </li>

                    <li class="dropdown hidden-xs" style="color: red; ">
                        <button class="navbar-account-btn" data-toggle="dropdown" aria-haspopup="true">

                            @if ($rows = DB::table('users')->where('id', $_SESSION['access'])->first())
                                <img class="rounded" width="36" height="36" src="{{asset('assets/image/'.$rows->photo) }}"
                                     alt="Admin">
                                <?php
                        ...
                        <ul class="dropdown-menu dropdown-menu-right">
                            <li><a></a></li>
                            <li>
                                <a id="logout" href="{{url('/updateUser/'.$_SESSION['access'])}}">
...
<div class="layout-main">
    ...
    <div class="layout-sidebar" >
        <div class="layout-sidebar-backdrop" style="background-color:black;"></div>
    ...
    <div class="layout-footer">
        <div class="layout-footer-body  ">
            <small class="version">Hamid Lemar Ltd</small>
            <small class="copyright"><?php echo date('Y'); ?>&nbsp;&copy; By <a href="https://zeersign.com"> ZeerSign Software Solutions </a></small>
        </div>
    </div>
</div>
...
<script src="{{ asset('assets/js/jquery.table2excel.js')}}" type="text/JavaScript"></script>
<script>
    $(function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                if ((data.errors)){
                    ...
                    $('#showRow').append('<tr role="row" class="odd"><td>'+data.quantity+'</td><td>'+data.net_price+'</td> <td>'+data.net_price+'</td> <td>'+data.sales_price+'</td> <td>'+data.sales_price+'</td></tr>');
                }
            },
        ...
        $.ajax({
            ...
            success: function (data) {
                if ((data.errors)){
                    ...
                    $('.error').addClass('hidden');
                    $('#showRowproduct').append('<tr role="row" class="odd"><td tabindex="0" class="sorting_1">'+data.product_name+'</td><td>'+data.product_s+'</td><td style="white-space: nowrap !important;"><button class="edit-modal btn btn-info" data-id="'+data.id+'"data-name="'+data.product_name+'"><span class="glyphicon glyphicon-edit"></span> Edit  </button> <button class="delete-modals btn btn-danger" data-id="'+data.id+'" data-name="'+data.product_name+'"><span class="glyphicon glyphicon-trash"></span> Delete </button> </td></tr>');
                }
            },
            headers: { 'X-XSRF-TOKEN' : $_token },
            contentType: false,
        ...
        $.ajax({
            ...
            success: function (data) {
                if ((data.errors)){
                    ...
                    $('.error').addClass('hidden');
                    $('#showRowcompany').append('<tr role="row" class="odd"><td tabindex="0" class="sorting_1">'+data.name+'</td><td>'+data.address+'</td><td>'+data.phone+'</td><td>'+data.p_contact+'</td><td>0</td><td>0</td><td>0</td><td>'+data.tin+'</td><td>'+data.lic+'</td><td>'+data.reg+'</td><td><button class="edit-modal btn btn-info" data-id="'+data.id+'" data-name="'+data.name+'"><span class="glyphicon glyphicon-edit"></span> Edit  </button>&nbsp;<button class="delete-modals btn btn-danger" data-id="'+data.id+'" data-name="'+data.name+'"><span class="glyphicon glyphicon-trash"></span> Delete </button> </td></tr>');
                }
            },
            headers: { 'X-XSRF-TOKEN' : $_token },
            contentType: false,
    ...
    $(document).on('click', '.edit-modalcustomer', function() {
        ...
        $('#n2').val($(this).data('name2'));
        $('#n3').val($(this).data('name3'));
        $('#n4').val($(this).data('name4'));
        $('#n5').val($(this).data('name5'));
        $('#n6').val($(this).data('name6'));
        $('#myModaldcustomer').modal('show');
    });
    $('.modal-footer').on('click', '.edit', function() {
        $.ajax({
            ...
            success: function(data) {
                $('.item' + data.id).replaceWith('<tr class="odd item ' + data.id +'"><td tabindex="0" class="sorting_1">'+data.name+'</td><td>'+data.address+'</td><td>'+data.phone+'</td><td>'+data.p_contact+'</td><td>0</td><td>0</td><td>0</td><td>'+data.tin+'</td><td>'+data.lic+'</td><td>'+data.reg+'</td><td style="white-space: nowrap !important;"> <button class="edit-modal btn btn-info" data-id="'+data.id+'" data-name="'+data.name+'"><span class="glyphicon glyphicon-edit"></span> Edit </button><button class="delete-modals btn btn-danger" data-id="'+data.id+'" data-name="'+data.name+'"><span class="glyphicon glyphicon-trash"></span> Delete</button></td></tr>');
            }
        });
    });
</script>
